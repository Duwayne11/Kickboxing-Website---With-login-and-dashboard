...
if (isset($_POST['reg_user'])) {
 ...
 // $sensei = mysqli_real_escape_string($db, $_POST['sensei']);
  ...
  if (empty($username)) { array_push($errors, "Username is required"); }  //name
  if (empty($surname)) { array_push($errors, "Surname is required"); }		//surname
...
//  if (empty($sensei)) { array_push($errors, "Gender is required"); }
 // if (empty($message)) { array_push($errors, "Gender is required"); }
  ...
  $user_check_query = "SELECT * FROM registration WHERE userID='$userID' OR email='$email' OR username='$username' LIMIT 1";
...
if (isset($_POST['btnMessage'])) {
  ...
  $message = mysqli_real_escape_string($db, $_POST['message']);
   $pubDate = mysqli_real_escape_string($db, $_POST['pubDate']);
  // Finally, register user if there are no errors in the form

  	$query2 = "INSERT INTO notification (senseiName, message, pubDate) VALUES('$sensei', '$message', '$pubDate')";
